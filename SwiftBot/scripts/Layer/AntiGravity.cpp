...
namespace GameLogic
{
    ...
    void AntiGravity::OnCollisionEnter(Frost::BodyOnContactParameters params, float deltaTime)
    {
        ...
        {
            GameState::Get().GetPlayerData(playerObject.GetParent()).isInAntiGravity = true;
        }
    }

    void AntiGravity::OnCollisionExit(std::pair<GameObject::Id, GameObject::Id> params, float deltaTime)
    {
        auto antiGravityHandle = GetGameObject().GetHandle();
        auto otherHandle = (params.first == antiGravityHandle) ? params.second : params.first;
        ...
        {
            GameState::Get().GetPlayerData(otherObject.GetParent()).isInAntiGravity = false;
        }
    }
} // namespace GameLogic