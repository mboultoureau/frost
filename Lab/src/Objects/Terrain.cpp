...
Terrain::Terrain()
{
    ...
    _scene.AddComponent<Transform>(
        _terrain, Vector3{ 0.0f, -20.0f, 0.0f }, EulerAngles{ 0.0f, 0.0f, 0.0f }, Vector3{ 1000.0f, 10.0f, 1000.0f });
    heightScale = 1.0f;
    ...
    HeightMapConfig heightMapConfig{
        material, AssetManager::LoadAsset(filepath, heightmapConfig), 50, transform->scale
    };

    // material.albedoTextures.push_back(AssetManager::LoadAsset<Texture>("./resources/textures/dirt.jpg",
    // "./resources/textures/dirt.jpg", TextureType::DIFFUSE));
    ...
    // auto texture =
    // static_cast<TextureDX11*>(Frost::TextureLibrary::Get().GetTexture(filepath,
    // TextureType::UNKNOWN).get()); ShapeRefC heightFieldShape =
    // Physics::Get().CreateHeightFieldShapeFromTexture(texture,
    // TextureChannel::R, heightScale, scale); ShapeRefC heightFieldShape =
    // BoxShapeSettings(Vec3(1000.0f, 20.0f, 1000.0f)).Create().Get();
    ShapeRefC heightFieldShape = ShapeFactory::CreateHeightMap(heightMapConfig);

    BodyCreationSettings bodySettings(
        heightFieldShape, RVec3(0, -500, 0), Quat::sIdentity(), EMotionType::Static, ObjectLayers::NON_MOVING);
