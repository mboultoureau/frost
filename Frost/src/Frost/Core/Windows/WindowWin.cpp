...
namespace Frost
{
    ...
    WindowWin::WindowWin(const WindowTitle title) : Window(title)
    {
        ...
        _resizeEventHandlerId =
            EventManager::SubscribeFront<WindowResizeEvent>(FROST_BIND_EVENT_FN(WindowWin::OnWindowResize));
    ...
    LRESULT CALLBACK WindowWin::_WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        switch (message)
        {
            ...
            case WM_MOUSEWHEEL:
            {
                ...
                Input::GetMouse().SetScroll(Mouse::MouseScroll{ Input::GetMouse().GetScroll().scrollX, delta });
                break;
            }
            case WM_MOUSEHWHEEL:
            {
                int16_t delta = GET_WHEEL_DELTA_WPARAM(wParam);
                Input::GetMouse().SetScroll(Mouse::MouseScroll{ delta, Input::GetMouse().GetScroll().scrollY });
                break;
            }
            default:
                return DefWindowProc(hWnd, message, wParam, lParam);
