...
namespace Frost
{
	...
	public:
		...
		static std::shared_ptr<Model> CreateSphere(float radius = 1.0f, uint32_t sliceCount = 32, uint32_t stackCount = 16);
		static std::shared_ptr<Model> CreatePlane(float width = 10.0f, float depth = 10.0f);
		static std::shared_ptr<Model> CreateCylinder(float bottomRadius = 1.0f, float topRadius = 1.0f, float height = 2.0f, uint32_t sliceCount = 32, uint32_t stackCount = 1);
	};
}