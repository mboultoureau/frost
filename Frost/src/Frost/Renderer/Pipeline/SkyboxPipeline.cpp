...
namespace Frost
{
    ...
    void SkyboxPipeline::Initialize()
    {
        ...
        ShaderDesc vsDesc = { .type = ShaderType::Vertex,
                              .debugName = "VS_Skybox",
                              .filePath = "../Frost/resources/shaders/VS_Skybox.hlsl" };
        ShaderDesc psDesc = { .type = ShaderType::Pixel,
                              .debugName = "PS_Skybox",
                              .filePath = "../Frost/resources/shaders/PS_Skybox.hlsl" };
        ...
        _vsSkyboxConstants = renderer->CreateBuffer(BufferConfig{ .usage = BufferUsage::CONSTANT_BUFFER,
                                                                  .size = sizeof(VS_SkyboxConstants),
                                                                  .dynamic = true,
                                                                  .debugName = "SB_VS_SkyboxConstants" });
    ...
    {
        Math::Vector3 vertices[] = { { -1.0f, -1.0f, 1.0f }, { 1.0f, -1.0f, 1.0f },   { 1.0f, 1.0f, 1.0f },
                                     { -1.0f, 1.0f, 1.0f },  { -1.0f, -1.0f, -1.0f }, { 1.0f, -1.0f, -1.0f },
                                     { 1.0f, 1.0f, -1.0f },  { -1.0f, 1.0f, -1.0f } };

        uint32_t indices[] = { 0, 1, 2, 2, 3, 0, 4, 7, 6, 6, 5, 4, 0, 3, 7, 7, 4, 0,
                               1, 5, 6, 6, 2, 1, 3, 2, 6, 6, 7, 3, 0, 4, 5, 5, 1, 0 };
        _cubeIndexCount = ARRAYSIZE(indices);

        auto* renderer = RendererAPI::GetRenderer();
        _cubeVertexBuffer = renderer->CreateBuffer(BufferConfig{ .usage = BufferUsage::VERTEX_BUFFER,
                                                                 .size = sizeof(vertices),
                                                                 .debugName = "SB_CubeVertexBuffer" },
                                                   vertices);
        _cubeIndexBuffer = renderer->CreateBuffer(BufferConfig{ .usage = BufferUsage::INDEX_BUFFER,
                                                                .size = sizeof(indices),
                                                                .debugName = "SB_CubeIndexBuffer" },
                                                  indices);
    }

    void SkyboxPipeline::Render(CommandList* commandList,
                                ...
                                const Component::WorldTransform& cameraTransform)
    {
        ...
        if (!skyboxTexture || !gbufferDepth || !renderTarget || !commandList)
            return;
