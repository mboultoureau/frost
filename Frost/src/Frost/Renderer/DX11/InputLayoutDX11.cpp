...
namespace Frost
{
		...
		: InputLayout(attributes, shader)
	{
		...
		FT_ENGINE_ASSERT(d3d11Shader, "InputLayoutDX11: Provided shader is not a valid DirectX 11 shader.");
		FT_ENGINE_ASSERT(shader.GetType() == ShaderType::Vertex, "InputLayoutDX11: Shader provided is not a vertex shader.");
		...
		for (const auto& attribute : attributes)
		{
			for (uint32_t i = 0; i < attribute.arraySize; i++)
			{
				...
				desc.InputSlotClass = attribute.isInstanced ? D3D11_INPUT_PER_INSTANCE_DATA : D3D11_INPUT_PER_VERTEX_DATA;
