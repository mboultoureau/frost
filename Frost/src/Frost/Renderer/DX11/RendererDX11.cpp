 ...
 */
    ...
    void RendererDX11::OnWindowResize(WindowResizeEvent& resizeEvent)
    {
        ...
        HRESULT hr = _swapChain->ResizeBuffers(2, newWidth, newHeight, DXGI_FORMAT_UNKNOWN, 0);
    ...
    void RendererDX11::EndFrame()
    {
        ...
        {
            FT_ENGINE_WARN("Invalid call when presenting swap chain probably "
                           "due to ALT+ENTER",
                           hr);
            return;
        }
    ...
    void RendererDX11::_CreateDevice()
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            // pInfoQueue->SetBreakOnSeverity(D3D11_MESSAGE_SEVERITY_WARNING,
            // TRUE);
    ...
    ID3D11DepthStencilState* RendererDX11::GetDepthStencilState(DepthMode mode) const
    {
        switch (mode)
        {
            case DepthMode::ReadOnly:
                return _depthStateReadOnly.Get();
            case DepthMode::None:
                return _depthStateNone.Get();
            case DepthMode::ReadWrite:
            default:
                return _depthStateReadWrite.Get();
        }
    }
} // namespace Frost