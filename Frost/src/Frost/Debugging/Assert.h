...
#define FT_ASSERT_INTERNAL_WITH_MSG(log_func, condition, custom_msg_format, ...) \
    do {                                                                         \
        if (!(condition)) {                                                      \
            log_func("Assertion failed: {} [{}:{}] | " custom_msg_format,        \
                #condition,                                                      \
                __FILE__,                                                        \
                __LINE__,                                                        \
                ##__VA_ARGS__);                                                  \
...
#define FT_ENGINE_ASSERT(...) \
    FT_EXPAND(FT_CHOOSE_ASSERT_MACRO(__VA_ARGS__)(::Frost::FT_ENGINE_CRITICAL, __VA_ARGS__))
}
