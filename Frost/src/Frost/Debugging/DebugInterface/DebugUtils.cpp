...
namespace Frost
{
	...
	bool DebugUtils::DrawVec3Control(const std::string& label, Math::Vector3& values, float resetValue, float columnWidth)
	{
		// Display values
	...
	bool DebugUtils::DrawQuaternionControl(const std::string& label, Math::Vector4& quaternion, float resetValue, float columnWidth)
	{
		EulerAngles eulerRadians = QuaternionToEulerAngles(quaternion);
		Vector3 eulerDegrees = {
			Angle<Degree>(eulerRadians.Pitch).value(),
		...
		ImGui::SameLine();
		if (ImGui::DragFloat("##Y", &eulerDegrees.y, 0.5f, -180.0f, 180.0f, format.c_str())) modified = true;
		ImGui::PopItemWidth();
		ImGui::SameLine();
		...
		ImGui::SameLine();
		if (ImGui::DragFloat("##Z", &eulerDegrees.z, 0.5f, -180.0f, 180.0f, format.c_str())) modified = true;
		ImGui::PopItemWidth();
