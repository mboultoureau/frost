...
namespace Frost
{
    ...
    void DebugScene::_DrawHierarchyPanel()
    {
        ...
        for (Scene* scene : _scenes)
        {
            if (ImGui::TreeNodeEx(scene->GetName().c_str(), ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                registry.view<Relationship, Meta>().each(
                    [&](auto entity, auto& relationship, auto& meta)
                    {
                    ...
                    if (!registry.all_of<Light>(_selectedEntity))
                    {
                        if (ImGui::MenuItem("Light"))
                        {
                            registry.emplace<Light>(
                                _selectedEntity, LightType::Point, Color3(1.0f, 1.0f, 1.0f), 1.0f, 10.0f);
                            ImGui::CloseCurrentPopup();
                        }
                    }
    ...
    void DebugScene::_DrawTransformComponent(Scene* scene, entt::entity gameObjectId)
    {
        ...
        if (DebugUtils::DrawComponentHeader("Transform (Local)"))
        {
            ...
            if (gameObjectId != _lastInspectedEntity)
            {
                ...
                _inspectorEulerCache = { Math::Angle<Math::Degree>(eulerRad.Pitch).value(),
    ...
    void DebugScene::_DrawWorldTransformComponent(Scene* scene, entt::entity gameObjectId)
    {
        ...
        if (DebugUtils::DrawComponentHeader("World Transform (Read-Only)"))
        {
            ...
            Math::Vector3 eulerAnglesDegrees{ Math::Angle<Math::Degree>(eulerAnglesRadians.Pitch).value(),
            ...
            ImGui::Text("Rotation (Quat): (%.2f, %.2f, %.2f, %.2f)",
            ...
            ImGui::Text("Rotation (Euler): (P:%.2f, Y:%.2f, R:%.2f) deg",
                        eulerAnglesDegrees.x,
                        eulerAnglesDegrees.y,
                        eulerAnglesDegrees.z);
        ...
        if (DebugUtils::DrawComponentHeader("Camera"))
        {
            ...
            if (camera->projectionType == Camera::ProjectionType::Perspective)
            {
                ...
                camera->perspectiveFOV =
                    Math::Angle<Math::Radian>{ Math::angle_cast<Math::Radian, Math::Degree>(currentFOVDegrees) };
            }
            else
            {
            ...
            if (ImGui::DragFloat("##X", &camera->viewport.x, 0.01f, 0.0f, 1.0f, "X:%.2f"))
            {
            }
            ImGui::SameLine();
            if (ImGui::DragFloat("##Y", &camera->viewport.y, 0.01f, 0.0f, 1.0f, "Y:%.2f"))
            {
            }
            ImGui::SameLine();
            if (ImGui::DragFloat("##W", &camera->viewport.width, 0.01f, 0.001f, 1.0f, "W:%.2f"))
            {
            }
            ImGui::SameLine();
    ...
    void DebugScene::_DrawVirtualCameraComponent(Scene* scene, entt::entity gameObjectId)
    {
        ...
        if (DebugUtils::DrawComponentHeader("Virtual Camera"))
        {
            ...
            if (ImGui::Combo(
                    "Respect Ratio", &currentRespectRatio, respectRatioOptions, IM_ARRAYSIZE(respectRatioOptions)))
            {
        ...
        if (DebugUtils::DrawComponentHeader("Rigid Body (Jolt)"))
        {
            ...
            if (lock.Succeeded())
            {
                ...
                if (body.GetMotionType() != JPH::EMotionType::Static)
                {
                    ...
                    {
                        // Physics::GetBodyInterface().SetLinearVelocity(rigidBody->bodyId,
                        // JPH::Vec3(linearVel[0], linearVel[1], linearVel[2]));
                    }
                    ...
                    {
                        // Physics::GetBodyInterface().SetAngularVelocity(rigidBody->bodyId,
                        // JPH::Vec3(angularVel[0], angularVel[1],
                        // angularVel[2]));
                    }
                    ...
                    {
                        // Physics::GetBodyInterface().SetGravityFactor(rigidBody->bodyId,
                        // gravity);
                    }
                }
            }
