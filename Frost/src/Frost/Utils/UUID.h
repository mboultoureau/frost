...
namespace Frost
{
	...
	*/
	...
	public:
		...
		static UUID<T> generate() noexcept
		{
			return UUID<T>(nextId());
		}
		...
		bool operator==(const UUID<T>& other) const noexcept
		{
		...
		struct Hash
		{
			std::size_t operator()(const UUID<T>& uuid) const noexcept
			{
				return std::hash<ValueType>()(uuid.value());
			}
		};
