...
namespace Frost::Math
{
    ...
    {
        static constexpr value_type to_neutral(value_type value) noexcept { return value; }

        static constexpr value_type from_neutral(value_type value) noexcept { return value; }

        static constexpr auto name() noexcept { return "Radian"sv; }

        static constexpr value_type neutral_value() noexcept { return 0.0f; }
    };
    ...
    struct angle_traits<Degree> : base_angle_traits
    {
        ...
        static constexpr auto name() noexcept { return "Degree"sv; }

        static constexpr value_type neutral_value() noexcept { return 0.0f; }
    };

    template<class Dest, class Src>
    constexpr typename angle_traits<Dest>::value_type angle_cast(
        const typename angle_traits<Src>::value_type&) noexcept;
    ...
    public:
        ...
        constexpr void swap(Angle& other) noexcept { std::swap(_value, other._value); }

        value_type value() const noexcept { return _value; }

        template<class U>
        constexpr Angle(const Angle<U>& other) noexcept : _value{ angle_cast<T, U>(other.value()) }
        {
        }
        ...
        constexpr bool operator!=(const Angle& other) const noexcept { return !(*this == other); }

        constexpr bool operator<(const Angle& other) const noexcept { return value() < other.value(); }

        constexpr bool operator<=(const Angle& other) const noexcept { return !(other.value() < value()); }

        constexpr bool operator>(const Angle& other) const noexcept { return other.value() < value(); }

        constexpr bool operator>=(const Angle& other) const noexcept { return !(value() < other.value()); }

        constexpr Angle operator+(const Angle& other) const noexcept { return { value() + other.value() }; }

        constexpr Angle operator-(const Angle& other) const noexcept { return { value() - other.value() }; }

        constexpr Angle operator*(value_type scalar) const noexcept { return { value() * scalar }; }
        ...
        constexpr Angle operator-() const noexcept { return { -value() }; }
    ...
    template<class Dest, class Src>
    constexpr typename angle_traits<Dest>::value_type angle_cast(
        const typename angle_traits<Src>::value_type& src) noexcept
    {
        return angle_traits<Dest>::from_neutral(angle_traits<Src>::to_neutral(src));
    }
