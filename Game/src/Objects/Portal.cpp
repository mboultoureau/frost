...
void PortalScript::OnInitialize()
{
	using namespace JPH;
...
void PortalScript::OnCollisionEnter(BodyOnContactParameters params, float deltaTime)
{
	if (!linkedPortalId.has_value()) return;
	if (portalType == PortalType::Exit) return;

	WarpPlayer();
}

void PortalScript::WarpPlayer()
{
	...
	JPH::Quat portal1Rotation{ portal1Transform->rotation.x, portal1Transform->rotation.y, portal1Transform->rotation.z, portal1Transform->rotation.w };
	JPH::Quat portal2Rotation{ portal2Transform->rotation.x, portal2Transform->rotation.y, portal2Transform->rotation.z, portal2Transform->rotation.w };
	JPH::Quat playerRotation{ playerTransform->rotation.x, playerTransform->rotation.y, playerTransform->rotation.z, playerTransform->rotation.w };
	JPH::Quat cameraRotation{ cameraTransform->rotation.x, cameraTransform->rotation.y, cameraTransform->rotation.z, cameraTransform->rotation.w };

	// Inverse rotation of portal 1
	JPH::Quat rotAInv = portal1Rotation.Conjugated();

	// --- POSITION ---
	JPH::Vec3 localPos = rotAInv * (Math::vector_cast<JPH::Vec3>(playerTransform->position) - Math::vector_cast<JPH::Vec3>(portal1Transform->position));
	...
	// --- OFFSET POSITION ---
	JPH::Vec3 offsetLocal = rotAInv * (Math::vector_cast<JPH::Vec3>(cameraTransform->position) - Math::vector_cast<JPH::Vec3>(playerTransform->position));
...
{
	Game::GetScene().AddComponent<Transform>(
		_portal,
		position,
		rotation,
		scale);
	Game::GetScene().AddComponent<WorldTransform>(
		_portal
	);
}

void Portal::SetupPortal(PortalType type, GameObject& otherPortal)
{
	Game::GetScene().AddScript<PortalScript>(
		_portal,
		_player->GetPlayerID(),
		type,
		otherPortal);
...
void Portal::SetupPortal(PortalType type)
{
	Game::GetScene().AddScript<PortalScript>(
		_portal,
		_player->GetPlayerID(),
